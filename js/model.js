...
model.resetPassword = (data) => {
  ...
  user
    ...
    .then(function () {
      user
        .updatePassword(data.password.value)
        .then(function () {
          model.updateDataToFireStore("users", {
            password: data.password.value,
          });
...
model.getInfoUser = async (email) => {
  ...
  if (data.docs[0] !== undefined) return data.docs[0].data();
  else return null;
};

model.getTest = async (user) => {
  const yourRooms = await firebase
    .firestore()
    .collection("rooms")
    .where("host", "==", user)
    .get();
  ...
  let db = model
    ...
    .onSnapshot(function (snapshot) {
      snapshot.docChanges().forEach(async function (change) {
        if (change.type === "added") {
          let friendImg = await model.getInfoUser(
            change.doc
              .data()
              .users.find((user) => user !== firebase.auth().currentUser.email)
          );
          ...
          if (
            change.doc
              .data()
              .users.find((item) => item == firebase.auth().currentUser.email)
          ) {
        ...
        if (change.type === "modified") {
          ...
          let modelConversation = model.allConversation.find(
            (item) => item.id == change.doc.id
          );
          if (model.currentConversation !== null) {
            if (
              change.doc.id == model.currentConversation.id &&
              messageData.length !== modelConversation.messages.length
            ) {
              ...
              if (
                ...
                html += view.addYourMessage(
                  messages[messages.length - 1].content
                );
              else
                html += view.addFriendMessage(
                  messages[messages.length - 1].content,
                  friendImg.photoURL
                );
